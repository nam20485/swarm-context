...
compatibility: Requires the swarm skill (.agents/skills/swarm/) and gh-issue-tracking-init (.agents/skills/gh-issue-tracking-init/) in the same repo, plus an interactive session (this wizard converses with the user).
---
...
1. **Logistics** (one combined ask): **slug** (kebab-case), **name** (display name),
   **repo** (target `owner/repo`, default = current repo), and target branch
   (default `dev/<slug>` per this repo's branching convention).
2. **Write the plan** to the canonical path `plan_docs/application_plan.md` — this
...
3. **Commit the plan** through the repo's `/safe-commit` flow (`git add plan_docs/`
   plus the safe-commit scan), so the committed plan is what the tracking init and
   every swarm task input reference.
4. **Derive swarm goal(s)** and ask for approval:
   ...
   checkable. Proceed only on explicit approval of the goal text.
5. **Initialize GH tracking** by invoking the `/gh-issue-tracking-init` skill against
...
6. **Start the swarm** by continuing into the `swarm` skill flow with the approved
...
Plan (phases → epics → tasks), Mandatory Requirements, Acceptance Criteria, Risk
Mitigation, Timeline, Success Metrics, Implementation Notes. Every Implementation Plan
task carries **Context**, **Acceptance Criteria**, and **Validation** — the issue
